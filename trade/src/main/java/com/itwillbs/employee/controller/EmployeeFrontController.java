...
public class EmployeeFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		else if (command.equals("/EmployeeMailList.emp")) {
			// 발송 메일 리스트
			action = new EmployeeSendMailList();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
		...
		else if (command.equals("/EmployeeMailContent.emp")) {
			// 발송 메일 컨텐츠
			action = new EmployeeSendMailContent();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
