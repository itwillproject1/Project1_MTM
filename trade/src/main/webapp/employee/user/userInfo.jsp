							...
							<div class="card-body">
								<ul class="nav nav-pills nav-fill mb-3" id="pills-tab"
									...
									<li class="nav-item"><a class="nav-link active"
										...
										aria-selected="true">기본 프로필</a></li>
									<li class="nav-item"><a class="nav-link"
										id="pills-trade-tab" data-toggle="pill" href="#pills-trade"
										role="tab" aria-controls="pills-trade" aria-selected="false">거래
											현황</a></li>
									<li class="nav-item"><a class="nav-link"
										id="pills-inquiry-tab" data-toggle="pill"
										href="#pills-inquiry" role="tab" aria-controls="pills-inquiry"
										aria-selected="false">문의 목록</a></li>
									<li class="nav-item"><a class="nav-link"
										id="pills-complain-tab" data-toggle="pill"
										href="#pills-complain" role="tab"
										aria-controls="pills-complain" aria-selected="false">신고 이력</a>
									</li>
								</ul>
								<div class="tab-content mb-1" id="pills-tabContent">
										...
										<div class="row align-items-center mb-4">
											<div class="col">
												<h2 class="h5 page-title">
													<small class="text-muted text-uppercase">${dto.user_nickname}</small><br>${dto.user_id}</h2>
											</div>
										</div>
										<div class="row mb-5">
												...
												<table class="table table-borderless table-striped">
													<tbody>
														<tr>
															...
															<th class="text-muted">${dto.user_name}</th>
															<td><strong>주소</strong></td>
															<th class="text-muted">${dto.address}</th>
														</tr>
														<tr>
															<td><strong>별명</strong></td>
															<th class="text-muted">${dto.user_nickname}</th>
															<td><strong>전화번호</strong></td>
															<th class="text-muted">${dto.phone}</th>
														</tr>
														<tr>
															<td><strong>생년월일</strong></td>
															<th class="text-muted">${dto.jumin}</th>
															<td><strong>이메일</strong></td>
															<th class="text-muted">${dto.email}</th>
														</tr>
														<tr>
															<td><strong>잔액</strong></td>
															<th class="text-muted">${dto.pay}</th>
															<td><strong>수신</strong></td>
															<th class="text-muted"><c:if
																	test="${dto.agree == '동의'}">
																	<span class="badge badge-pill badge-primary">동의</span>
																</c:if> <c:if test="${dto.agree == '비동의'}">
														...
														<c:if test="${dto.suspended}">
															<tr>
																...
																<th class="text-muted"><span
																	class="badge badge-pill badge-danger">진행 중</span></th>
																<td><strong>남은 기간</strong></td>
																<th class="text-muted">${dto.sus_days}</th>
															</tr>
														</c:if>
													</tbody>
												</table>
													...
													<tbody>
														<c:forEach var="i" items="${tlist}">
															<tr>
																...
																<td>[${i.deal_way}]${i.title}<br> <span
																	class="small text-muted">${i.content}</span>
																</td>
																<td class="text-right">${i.price}</td>
																<td class="text-right"><c:if
																		test="${i.deal_status == 1}">
																		<span class="badge badge-pill badge-success">거래
																			중</span>
																	</c:if> <c:if test="${i.deal_status == 0}">
																		<span class="badge badge-pill badge-secondary">거래
																			완료</span>
																	</c:if></td>
																<td class="text-right"><c:if
																		test="${i.deal_status == 0}">${i.deal_user_id}</c:if>
																</td>
															</tr>
														</c:forEach>
													</tbody>
									...
									<div class="tab-pane fade" id="pills-inquiry" role="tabpanel"
										aria-labelledby="pills-inquiry-tab">
										<!-- 문의 목록 -->
										<div class="row align-items-center mb-10">
											<div class="col">
												<h2 class="h5 page-title">
													...
													<tbody>
														<c:forEach var="i" items="${ilist}">
															<tr>
																...
																<td><c:if test="${i.complete}">
																		<span class="badge badge-pill badge-success">답변
																			완료</span>
																	</c:if> <c:if test="${!i.complete}">
										...
										<div class="row align-items-center mb-10">
											...
											<div class="col-auto">
												<button type="button" class="btn btn-secondary"
													onclick="location.href='./UserSuspendCancelForm.emp?user_id=${dto.user_id}'">정지
													취소</button>
												<button type="button" class="btn btn-primary"
													onclick="location.href='./UserSuspendActiveForm.emp?user_id=${dto.user_id}'">유저
													정지</button>
											</div>
										</div>
										<div class="row mb-12">
													...
													<tbody>
														<c:forEach var="i" items="${clist}">
															<tr>
																...
																<td>${i.uploadDate}</td>
																<td class="text-right"><c:if test="${i.complete}">
																		<span class="badge badge-pill badge-success">처리
																			완료</span>
																	</c:if> <c:if test="${!i.complete}">
