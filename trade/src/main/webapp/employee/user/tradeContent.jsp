								...
								<ul class="nav nav-pills nav-fill mb-3" id="pills-tab"
									...
									<c:if test="${suggestCount > 0}">
										<li class="nav-item"><a class="nav-link"
											...
											aria-controls="pills-profile" aria-selected="false">거래 제안</a>
										</li>
									</c:if>
								</ul>
												...
												<table class="table table-borderless table-striped"
													...
													<tbody>
														<tr>
															...
															<th class="text-muted">${dto.bno}</th>
															<td><strong>아이디</strong></td>
															<th class="text-muted">${dto.user_id}</th>
														</tr>
														<tr>
															<td><strong>카테고리</strong></td>
															<th class="text-muted">${dto.category}</th>
															<td><strong>등록일</strong></td>
															<th class="text-muted">${dto.date_time}</th>
														</tr>
														<tr>
															<td><strong>브랜드</strong></td>
															<th class="text-muted">${dto.brand}</th>
															<td><strong>가격</strong></td>
															<th class="text-muted">${dto.price}</th>
														</tr>
														<tr>
															<td><strong>거래 방식</strong></td>
															<th class="text-muted">${dto.deal_way}</th>
															<td><strong>거래 제안</strong></td>
															<th class="text-muted">${suggestCount}</th>
														</tr>
														<tr>
															<td><strong>거래 상태</strong></td>
															<th class="text-muted"><c:if
																	test="${dto.deal_status == 1}">
																	<span class="badge badge-pill badge-success">거래
																		중</span>
																</c:if> <c:if test="${dto.deal_status == 0}">
																	<span class="badge badge-pill badge-secondary">거래
																		완료</span>
																</c:if></th>
															<td><strong>좋아요</strong></td>
															<th class="text-muted">${dto.like_count}</th>
														</tr>
													</tbody>
												</table>
											</div>
															...
															<td>
																<p class="mb-0 text-muted">
																	<c:if test="${empty i.seller_id}">
																		<a href="./UserInfo.emp?user_id=${i.buyer_id}"
																			class="text-muted">${i.buyer_id}</a>
																	</c:if>
																	<c:if test="${empty i.buyer_id}">
																		<a href="./UserInfo.emp?user_id=${i.seller_id}"
																			class="text-muted">${i.seller_id}</a>
																	</c:if>
																</p>
															</td>
															<td>
																<p class="mb-0 text-muted">
																	<a href="./TradeContent.emp?bno=${i.bno}"
																		class="text-muted"> ${i.title} </a>
																</p>
															</td>
															<td><c:if test="${empty i.seller_id}">${i.buyer_price}</c:if>
