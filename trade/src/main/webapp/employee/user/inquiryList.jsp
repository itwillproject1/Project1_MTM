			...
			<div class="col-12" data-select2-id="6">
				...
				<div class="row mb-4 items-align-center">
					...
					<div class="col-md-auto ml-auto text-right">
						...
						<button type="button" class="btn" onclick="./InquiryList.emp">
									...
									<div class="p-2">
										...
										<div class="form-group my-2">
											...
											<label for="custom-select"><strong>카테고리</strong></label> <select
												...
												<c:if test="${category == '계정문의'}">
													<option value="1" selected>계정문의</option>
												</c:if>
												<c:if test="${category != '계정문의'}">
													<option value="1">계정문의</option>
												</c:if>
												<c:if test="${category == '구매문의'}">
													<option value="2" selected>구매문의</option>
												</c:if>
												<c:if test="${category != '구매문의'}">
													<option value="2">판매문의</option>
												</c:if>
												<c:if test="${category == '판매문의'}">
													<option value="3" selected>판매문의</option>
												</c:if>
												<c:if test="${category != '판매문의'}">
													<option value="3">거래</option>
												</c:if>
												<c:if test="${category == '결제문의'}">
													<option value="4" selected>결제문의</option>
												</c:if>
												<c:if test="${category != '결제문의'}">
						...
						<tbody>
							<c:forEach var="i" items="${list}">
								<tr>
									...
									<td>
										<p class="mb-0 text-muted">
											<a href="./InquiryContent.emp?bno=${i.bno}"
												class="text-muted">${i.subject}</a>
										</p>
									</td>
									<td>${i.content}</td>
									<td>${i.uploadDate}</td>
									<td>
										<!-- 답변 완료 --> <c:if test="${i.complete}">
											<span class="dot dot-lg bg-success mr-2"></span>
										</c:if> <!-- 답변 x --> <c:if test="${!i.complete}">
											<span class="dot dot-lg bg-secondary mr-2"></span>
										</c:if>
									</td>
									<td><c:if test="${i.complete}">${i.emp_id}</c:if></td>
									<td>
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-right" style="">
												<a class="dropdown-item"
													href="./InquiryContent.emp?bno=${i.bno}">상세보기</a>
											</div>
										</div>
									</td>
								</tr>
